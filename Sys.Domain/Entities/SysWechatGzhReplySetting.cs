...
namespace Sys.Domain.Entities
{
    ...
    /// 微信公众号回复设置
    /// </summary>
    public class SysWechatGzhReplySetting
    {
        /// <summary>
        /// 数据id
        /// </summary>
        ...
        public void SetXmlContent()
        {
            switch (ReplyType)
            {
                case SysWechatGzhReplyTypeEnum.Text: XmlContent = GetTextXmlContent(); break;
                case SysWechatGzhReplyTypeEnum.Image: XmlContent = GetImageXmlContent(); break;
                case SysWechatGzhReplyTypeEnum.Voice: XmlContent = GetVoiceXmlContent(); break;
                case SysWechatGzhReplyTypeEnum.Video: XmlContent = GetVideoXmlContent(); break;
                case SysWechatGzhReplyTypeEnum.Music: XmlContent = GetMusicXmlContent(); break;
                case SysWechatGzhReplyTypeEnum.Article: XmlContent = GetArticleXmlContent(); break;
                case SysWechatGzhReplyTypeEnum.CustomerService: XmlContent = GetCustomerServiceJsonContent(); break;
            }
        }
        ...
        public string GetXmlReplyContent(string toUser, string fromUser)
        {
            ...
            if (!ContentJson.IsNullOrEmpty() && !XmlContent.IsNullOrEmpty())
            {
                ...
                var contents = ContentJson.FromJson<IEnumerable<SysWechatGzhReplySettingContentVo>>();
                contents.ForEach(e =>
                {
                    xmlContent = xmlContent.Replace($"[{e.Name}]", $"[{e.Value}]");
                });
                xmlContent = xmlContent.Replace($"[createtime]", $"[{DateTime.UtcNow.ToString("yyyyMMddHHmmss")}]");
