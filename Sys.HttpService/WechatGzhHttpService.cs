...
namespace Sys.HttpService
{
    ...
    public class WechatGzhHttpService : BaseHttpService, IWechatGzhHttpService
    {
        ...
        public async Task<WechatUnionIdResponse> GetUnionIdAsync(string openid, string accessToken)
        {
            var client = GetHttpClient(_config.Wxgzh);
            if (client != null && client.BaseAddress != null)
            {
                var url = $"cgi-bin/user/info?access_token={accessToken}&openid={openid}&lang=zh_CN";
                var result = await client.GetAsync(url);
                var str = await result.Content.ReadAsStringAsync();
                return str.FromJson<WechatUnionIdResponse>();
            }
            return null;
        }
